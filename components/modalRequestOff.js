...
export default function modalRequestOff({isOpen, setClose}) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const response = await offDay.submit(JSON.parse(localStorage.getItem('user')).employee_id, typeOff,date1.format('YYYY-MM-DDTHH:mm:ssZ[Z]'),date2.format('YYYY-MM-DDTHH:mm:ssZ[Z]'),date2.diff(date1, 'day', false),notes)
